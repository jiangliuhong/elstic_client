        ...
        >
          ...
          <div class="server-content" :style="{ height: tabContentHeight }">
                ...
                <n-space vertical style="padding-bottom: 16px">
                  ...
                  <div v-show="queryTab.type === 'basic'">
                    <n-space vertical>
                      ...
                      <!-- 第二行：查询条件 -->
                      <div v-for="(condition, index) in queryTab.basicForm.conditions" :key="index">
                        <n-space align="center">
                          ...
                          <n-select 
                            ...
                            filterable
                            style="width: 150px"
                            @update:value="(val) => handleFieldChange(serverTab.id, queryTab.id, index, val)"
                          />
                          ...
                          <n-input 
                            v-if="condition.operator && ['match', 'term', 'wildcard', 'query_string', 'text'].includes(condition.operator)"
                          ...
                          <!-- 5. 添加和删除按钮 -->
                          <n-button @click="addCondition(serverTab.id, queryTab.id)" size="small" type="primary">添加</n-button>
                          <n-button @click="removeCondition(serverTab.id, queryTab.id, index)" size="small" type="error" 
                                   :disabled="queryTab.basicForm.conditions.length <= 1">删除</n-button>
                        </n-space>
                      </div>
                    </n-space>
                  </div>
                  ...
                  <n-space>
                    <n-button @click="runQuery(serverTab.id, queryTab.id)" type="primary">执行查询</n-button>
                    <n-button @click="saveQuery(serverTab.id, queryTab.id)">保存查询</n-button>
                  </n-space>
                  
                  <n-card title="查询结果" v-show="queryTab.result && queryTab.result.length > 0">
                    <n-data-table
            ...
            <n-modal v-model:show="showAddQueryModal" preset="dialog" title="新增查询">
              ...
              <n-form :model="newQueryForm" label-placement="left" label-width="100">
...
import { NTabs, NTabPane, NResult, NButton, NModal, NForm, NFormItem, NInput, NSelect, NSpace, NRadioGroup, NRadioButton, NCard, NDataTable } from 'naive-ui'
...
const addServerTab = (server: Server) => {
  ...
  const initialShardData = [
    ...
    { index: 'order_history', shard: '0', prirep: 'p', type: 'primary', status: 'STARTED', node: 'node-1' },
    { index: 'order_history', shard: '1', prirep: 'r', type: 'replica', status: 'STARTED', node: 'node-2' },
    { index: 'user_logs', shard: '0', prirep: 'p', type: 'primary', status: 'STARTED', node: 'node-3' },
    { index: 'user_logs', shard: '1', prirep: 'r', type: 'replica', status: 'STARTED', node: 'node-1' }
  ];
  
  // 从分片数据中提取唯一的索引
  const uniqueIndexes = [...new Set(initialShardData.map(item => item.index))];
...
const handleAddQuery = () => {
  ...
  const activeServerTab = serverTabs.value.find(tab => tab.id === activeTabId.value)
  if (activeServerTab) {
...
// 处理字段变化
const handleFieldChange = (serverId: string, tabId: string, conditionIndex: number, field: string) => {
  const tab = serverTabs.value.find(t => t.id === serverId)
  if (tab) {
...
// 删除查询条件
const removeCondition = (serverId: string, tabId: string, conditionIndex: number) => {
  const tab = serverTabs.value.find(t => t.id === serverId)
  if (tab) {
    const queryTab = tab.queryTabs.find(qt => qt.id === tabId)
